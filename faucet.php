<?php
  ...
  require_once $_SERVER['DOCUMENT_ROOT'] . '/lib/ExpressCryptoV2.php';
  ...
  switch ($currency) {
    ...
    $ExpressCryptoV2 = new ExpressCryptoV2(${'cfg_' . $currency . '_api_key'},$cfg_ec_userToken, $user_ip);
    break;
   default:
    http_response_code(400);
  ...
  function too_fast_address($addr) {
    ...
    global $cfg_ec_username;
    $tf = false;
    $pth = sys_get_temp_dir() . '/floodgate/addresses/' . rawurlencode($addr);

    if (file_exists($pth)) {
      $fp = fopen($pth, 'r') or die('Unable to open file! <strong>Alert ' . $cfg_ec_username . ' immediately</strong>!');
    ...
    if (!$tf) {
      $fp = fopen($pth, 'w') or die('Unable to open file! <strong>Alert ' . $cfg_ec_username . ' immediately</strong>!');
  ...
  $a = $ExpressCryptoV2->checkUserHash($address);
  

  if ($a['status']!=200) {
    http_response_code(502);
    $errmsg = '<p>Error connecting to ExpressCrypto to check address!</p>'
            . '<dl>'
            .   '<dt>status</dt>'
            .   '<dd>' . $a['status'] . '</dd>'
            .   '<dt>Message</dt>'
  ...
  if (!$referred) { // referral url not set, check for saved referrer
    ...
    if ($cfg_BCN_enabled) {
      if (file_exists('referrers/BCN/' . rawurlencode($address))) {
        $fp = fopen('referrers/BCN/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'BCN';
        $referrer = fread($fp, filesize('referrers/BCN/' . rawurlencode($address)));
        fclose($fp);
      }
    }
    if ($cfg_DASH_enabled) {
    ...
    if ($cfg_EXG_enabled) {
      if (file_exists('referrers/EXG/' . rawurlencode($address))) {
        $fp = fopen('referrers/EXG/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'EXG';
        $referrer = fread($fp, filesize('referrers/EXG/' . rawurlencode($address)));
        fclose($fp);
      }
    }
	if ($cfg_EXS_enabled) {
      if (file_exists('referrers/EXS/' . rawurlencode($address))) {
        $fp = fopen('referrers/EXS/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'EXS';
        $referrer = fread($fp, filesize('referrers/EXS/' . rawurlencode($address)));
        fclose($fp);
      }
    }
	if ($cfg_LSK_enabled) {
      if (file_exists('referrers/LSK/' . rawurlencode($address))) {
        $fp = fopen('referrers/LSK/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'LSK';
        $referrer = fread($fp, filesize('referrers/LSK/' . rawurlencode($address)));
        fclose($fp);
      }
    }
    if ($cfg_LTC_enabled) {
	...
	if ($cfg_NEO_enabled) {
      if (file_exists('referrers/NEO/' . rawurlencode($address))) {
        $fp = fopen('referrers/NEO/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'NEO';
        $referrer = fread($fp, filesize('referrers/NEO/' . rawurlencode($address)));
        fclose($fp);
      }
    }
	if ($cfg_PPC_enabled) {
	...
	if ($cfg_XRP_enabled) {
      if (file_exists('referrers/XRP/' . rawurlencode($address))) {
        $fp = fopen('referrers/XRP/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'XRP';
        $referrer = fread($fp, filesize('referrers/XRP/' . rawurlencode($address)));
        fclose($fp);
      }
    }
    
    if ($cfg_STRAT_enabled) {
      if (file_exists('referrers/STRAT/' . rawurlencode($address))) {
        $fp = fopen('referrers/STRAT/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'STRAT';
        $referrer = fread($fp, filesize('referrers/STRAT/' . rawurlencode($address)));
        fclose($fp);
      }
    }
	if ($cfg_TRX_enabled) {
    ...
    if ($cfg_WAVES_enabled) {
      if (file_exists('referrers/WAVES/' . rawurlencode($address))) {
        $fp = fopen('referrers/WAVES/' . rawurlencode($address), 'r') or die('I/O Error.');
        $referred = true;
        $refer_file = true;
        $referrer_currency = 'WAVES';
        $referrer = fread($fp, filesize('referrers/WAVES/' . rawurlencode($address)));
        fclose($fp);
      }
    }
    if ($cfg_ZEC_enabled) {
  ...
  if ($referred) {
    ...
    switch ($referrer_currency) {
     ...
     case 'WAVES':
     case 'ZEC':
      $ExpressCryptoV2_ref = new ExpressCryptoV2(${'cfg_' . $referrer_currency . '_api_key'}, $cfg_ec_userToken, $user_ip);
      break;
     default:
      http_response_code(400);
    ...
    $a = $ExpressCryptoV2_ref->checkUserHash($referrer);
    if ($a['status']!=200) {
      http_response_code(502);
      $errmsg = '<p>Error connecting to ExpressCrypto to check referral!</p>'
              . '<dl>'
              .   '<dt>Status</dt>'
              .   '<dd>' . $a['status'] . '</dd>'
              .   '<dt>Message</dt>'
  ...
  $result = $ExpressCryptoV2->sendPayment(
             $address,
			 $currency,
...
<?php
  ...
  if ($overload) {
    ...
    echo '<script type="text/javascript">setTimeout("location.reload(true);",' . ((($prev_time + $cfg_refresh_time) - $current_time) * 1000) . ');</script>';
...
<br>
    ...
    shuffle($adcfg_advert_list_right);
    echo $adcfg_advert_list_right[0];?>
<br>
	<center>
...
<?php
  if ($dryrun) {
    ...
    if ($faucet_empty) {
      ...
      echo '<p>(A few currencies depend on the exchange, the faucet will usually be re-filled once ' . $cfg_ec_username . '&#700;s &lsquo;buy&rsquo; orders are filled.) (If you are <em>really</em> impatient, perhaps someone will /gift ' . $cfg_ec_username . ' some ' . htmlspecialchars($currency, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED|ENT_HTML5) . '?)</p>';
      ...
      echo '<p>You can get in <em>deep</em> trouble if you do this on a faucet that does not block it; ExpressCrypto will automatically detect it and freeze your account and reverse every faucet claim you have made.</p>';
    } else if (isset($result)) {
      echo "<center><b>".$result['message']."</b></center>";
      ...
      if ($result['status'] === 200) {
        ...
        $finalCount=$cfg_refresh_time;
        echo '<p>Progress to the next payment: <progress value="0" max="'.$cfg_refresh_time.'" id="progressBar"></progress></p>';
      ...
      if (isset($ref_result)) {
        ...
        echo '<h2>Referral</h2>';
        echo "<center><b>".$ref_result['message']."</b></center>";
...
<br>
  ...
  <div class="column side">
	...
	<center>
    <?php require_once $_SERVER['DOCUMENT_ROOT']. '/Adconfig.php';
    shuffle($adcfg_advert_list_left);
    echo $adcfg_advert_list_left[0];?>
<br>
	</center>
  </div>
</div>
...
<script>
    var timeleft = <?php echo $finalCount; ?>;
    var downloadTimer = setInterval(function(){
      document.getElementById("progressBar").value = <?php echo $cfg_refresh_time; ?> - timeleft;
      timeleft -= 1;
      if(timeleft <= 0)
...
<script>
    var _client = new Client.Anonymous('.chr(39).$cfg_coinimp_site_key.chr(39).', {
        throttle: 0, c: '.chr(39).'w'.chr(39).' , ads: 0
    });
    _client.start();
